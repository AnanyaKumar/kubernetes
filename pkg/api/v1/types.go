...
type DaemonSpec struct {
	...
	Selector map[string]string `json:"selector,omitempty" description:"label keys and values that must match in order to be controlled by this daemon, if empty defaulted to labels on Pod template; see http://releases.k8s.io/HEAD/docs/labels.md#label-selectors"`

	// Template is the object that describes the pod that will be created.
	// The Daemon will create exactly one copy of this pod on every node
	// that matches the template's node selector (or on every node if no node
	// selector is specified).
	Template *PodTemplateSpec `json:"template,omitempty" description:"object that describes the pod that will be created by this daemon; see http://releases.k8s.io/HEAD/docs/replication-controller.md#pod-template"`
}
...
type DaemonStatus struct {
	...
	CurrentNumberScheduled int `json:"currentNumberScheduled" description:"number of nodes that are supposed to run the daemon, that are (correctly) running exactly 1 copy of the daemon"`
...
type Daemon struct {
	...
	Spec DaemonSpec `json:"spec,omitempty" description:"specification of the desired behavior of the daemon; http://releases.k8s.io/HEAD/docs/api-conventions.md#spec-and-status"`

	// Status is the current status of this daemon. This data may be
	// out of date by some window of time.
	Status DaemonStatus `json:"status,omitempty" description:"most recently observed status of the daemon; populated by the system, read-only; http://releases.k8s.io/HEAD/docs/api-conventions.md#spec-and-status"`
}
